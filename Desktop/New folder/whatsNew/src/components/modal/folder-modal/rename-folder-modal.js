...
function RenameFolderModal(props) {
  ...
  const titleKey = "files:create.folder.modal:rename.folder";
  const modalTitle = t(titleKey);

  const [inputs, setInputs] = useState({
    name: props.folderName,
    description: props.folderDesc,
  });
  ...
  const renameFolderApiError = useSelector(
    (state) => state.folderReducer.renameFolderApiError
  );
  const { updateFolderSuccess } = useSelector((state) => state.folderReducer);
  const maxChars = 64;
  ...
  function handleSubmit(e) {
    ...
    if (createFolderApiError) {
      dispatch(resetUpdateFolderAction());
    }
    setSubmitted(true);
    let folder = {};
    name = name.trim();
    if (name) {
      ...
      folder.type = "BASIC";
      dispatch(updateFolderByFolderId(props.folderId, folder, dispatch));
    }
  }

  useEffect(() => {
    if (updateFolderSuccess) {
      handleClose();
    }
  }, [updateFolderSuccess]);
      ...
      >
                ...
                <div className="form-wrapper">
                  <Form.Group className="m-0">
                    ...
                    >
                      ...
                      {submitted &&
                        renameFolderApiError &&
                        renameFolderApiError.code === 40024 && (
                          <div className="invalid-feedback">
                            {t(
                              "files:create.folder.modal:createFolderApiError:40024"
                            )}
        ...
        <Modal.Footer style={{ border: "none" }}>
          ...
          >
            ...
            {t("files:create.folder.modal:ok")}
          </button>
        </Modal.Footer>
      </StyledModal>{" "}
    </>
