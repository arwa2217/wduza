...
const Files = () => {
  ...
  let memoryUsed = bytesToSize(
    channelFilesStorage && channelFilesStorage.fileQuotaUsed,
    2
  );
  ...
  let freeMemory = bytesToSize(
    parseInt(channelFilesStorage && channelFilesStorage.fileQuotaAllowed) -
      parseInt(channelFilesStorage && channelFilesStorage.fileQuotaUsed)
  );
  ...
  const redirectToPost = (elem) => {
    ...
    if (elem) {
      data = elem;
      if (channelDetails.id && channelDetails.name && data.postUUID) {
        let postId = data
          ? data.ParentPostID
            ? data.ParentPostID
            : data.postUUID
  ...
  useEffect(() => {
    if (channelDetails.id) {
      ...
      currentChannel.current = channelDetails;
      return () => {
