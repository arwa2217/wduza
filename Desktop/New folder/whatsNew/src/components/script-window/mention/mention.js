...
import "./mention.css";
import "./mention-blot";
import DefaultUser from "@icons/default-user-mention.svg";
import { store } from "../../../store/store";
import { USER_MENTION } from "../../../store/actionTypes/user-mention-action-types";
...
class Mention {
  constructor(quill, options) {
    ...
    this.options = {
      source: null,
      handleShowAvatar(item) {
        const handleUserType = function (userType) {
          switch (userType) {
            case 'E':
              return `<img class="mention-user-list-item-type" src="/public/assets/v2/external.svg" alt=""/>`
            case 'G':
              return `<img class="mention-user-list-item-type" src="/public/assets/v2/ic_badge_guest.svg" alt=""/>`
            default:
              return `<span class="mention-user-list-item-type"></span>`
          }
        }
        return html`
          ${handleUserType(item.userType.substr(0, 1))}
        `
      },
      handleInfo(item) {
        if (item.userType.substr(0, 1) === 'E') {
          return html`
      ...
      renderItem(item) {
        return `
        <div class="mention-user-list-item">
          <div class="mention-user-list-item-avatar">
            ${item.userImg === "" 
            ?  `<img class="mention-user-list-item-img-default" src=${DefaultUser} />`
            : `<img class="mention-user-list-item-img" src=${item.userImg} />`
        }
            ${this.handleShowAvatar(item)}
          </div>
          <div class="mention-user-list-item-info">
            <span class="mention-user-list-item-name">${item.screenName}</span> 
            <div>
  ...
  insertItem(data, programmaticInsert) {
    ...
    if (!this.options.showCommandChar) {
      render.commandChar = "";
    }
  ...
  renderList(mentionChar, data, searchTerm, channelId) {
    if (data && data.length > 0) {
      ...
      for (let i = 0; i < data.length; i += 1) {
        ...
        li.dataset.commandChar = mentionChar;
        this.mentionList.appendChild(
          insertData(li, data[i], this.options.dataAttributes, channelId)
        );
      }
  ...
  setMentionContainerPosition_Fixed() {
    ...
    console.log("mentionCharPosAbsolute", mentionCharPosAbsolute)
    ...
    if (this.options.fixMentionsToQuill) {
      ...
      //if its off the right edge, push it back
      if (
    ...
    if (this.options.defaultMenuOrientation === "top" && fitsTop) {
      ...
      //it doesn't fit either so put it where there's the most space
    ...
    if (placement === "bottom") {
      ...
      if (!fitsBottom) {
        //shrink it to fit
        //3 is a bit of a fudge factor so it doesn't touch the edge of the screen
  ...
  onSomethingChange() {
    ...
    const { mentionChar, mentionCharIndex } = getCommandCharIndex(
      textBeforeCursor,
      this.options.mentionCommandChars
    );
    if (
      checkCommandCharIndexValidity(
        mentionCharIndex,
      ...
      if (
        ...
        this.options.source(
          textAfter,
          (data, searchTerm, channelId) => {
            if (sourceRequestToken.abandoned) {
              return;
            }
            this.existingSourceExecutionToken = null;
            this.renderList(mentionChar, data, searchTerm, channelId);
          },
          mentionChar
        );
      } else {
  ...
  openMenu(commandChar) {
    var selection = this.quill.getSelection(true);
    this.quill.insertText(selection.index, commandChar);
    this.quill.blur();
    this.quill.focus();
  }
}
